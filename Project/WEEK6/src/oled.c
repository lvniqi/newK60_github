...
作    者   : Guokaiyi
修改内容   : 修复F6x8字符数组的bug，
该bug会导致OLED_P6x8Char，OLED_P6x8Str显示错误
...
const unsigned char F14x16[] = {
  0x00, 0x80, 0xE0, 0x1C, 0x12, 0x10, 0x50, 0x52, 0x4C, 0x50, 0x50, 0x30, 0x08,
    0x00, 0x01, 0x00, 0x7F, 0x00, 0x00, 0x01, 0x7D, 0x45, 0x45, 0x45, 0x45,
    0x7C, 0x00, 0x00,  /*"信",0*/
  0x00, 0x20, 0x20, 0xA8, 0xA8, 0xB8, 0xA6, 0xA8, 0xB8, 0x68, 0xA8, 0x20, 0x20,
    0x00, 0x00, 0x40, 0x30, 0x0F, 0x76, 0x46, 0x4E, 0x56, 0x46, 0x76, 0x17,
    0x10, 0x60, 0x00,  /*"意",1*/
  0x00, 0x00, 0xF0, 0x90, 0x90, 0x90, 0xFE, 0x90, 0x90, 0x90, 0xF0, 0x10, 0x00,
    0x00, 0x00, 0x00, 0x0F, 0x08, 0x08, 0x08, 0x7F, 0x48, 0x48, 0x48, 0x4F,
    0x50, 0x60, 0x00,  /*"电",2*/
  0x00, 0x00, 0x04, 0x04, 0x04, 0x04, 0x04, 0xE4, 0x14, 0x0C, 0x04, 0x84, 0x80,
    0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x41, 0x41, 0x3F, 0x01, 0x01, 0x01,
    0x00, 0x00, 0x00,  /*"子",3*/
  0x00, 0x00, 0x48, 0xFC, 0x44, 0x42, 0x00, 0x88, 0x30, 0x00, 0xFE, 0x02, 0x00,
    0x00, 0x00, 0x0C, 0x02, 0x7F, 0x01, 0x01, 0x08, 0x08, 0x05, 0x04, 0xFF,
    0x04, 0x02, 0x00,  /*"科",4*/
  0x00, 0x00, 0x20, 0xFE, 0x20, 0x90, 0x20, 0x20, 0x20, 0xFE, 0x20, 0xA0, 0x10,
    0x00, 0x00, 0x02, 0x41, 0x7F, 0x81, 0x80, 0x41, 0x47, 0x29, 0x18, 0x27,
    0x41, 0x40, 0x00,  /*"技",5*/
  0x00, 0x00, 0xFE, 0x14, 0x14, 0x34, 0x54, 0x14, 0x94, 0x74, 0x14, 0x9C, 0x00,
    0x00, 0x00, 0x30, 0x0F, 0x88, 0x48, 0x39, 0x0F, 0x09, 0x08, 0x7F, 0x09,
    0x04, 0x04, 0x00,  /*"屏",6*/
  0x00, 0x04, 0x04, 0xF4, 0x54, 0xCE, 0x54, 0x54, 0x4E, 0x4C, 0xF4, 0x04, 0x04,
    0x00, 0x00, 0x08, 0x0A, 0x3E, 0x0A, 0x09, 0x7E, 0x0A, 0x0A, 0x7A, 0x0E,
    0x0A, 0x09, 0x00,  /*"幕",7*/
  0x00, 0x20, 0x04, 0x80, 0xFC, 0x04, 0xF4, 0x04, 0xFC, 0x00, 0xF0, 0xFC, 0x00,
    0x00, 0x00, 0x00, 0x7F, 0x40, 0x27, 0x18, 0x07, 0x10, 0x67, 0x00, 0x4F,
    0xFF, 0x00, 0x00,  /*"测",8*/
  0x00, 0x80, 0x88, 0x98, 0x20, 0x20, 0x20, 0x20, 0xA0, 0xFE, 0x24, 0x2C, 0x10,
    0x00, 0x00, 0x00, 0x1F, 0x60, 0x10, 0x28, 0x21, 0x1F, 0x10, 0x13, 0x1C,
    0x20, 0x70, 0x00,  /*"试",9*/
};
 ...
 * 内部函数原型                                                               *
 *----------------------------------------------------------------------------*/
void OLED_WrDat(unsigned char ucData);
void OLED_WrCmd(unsigned char ucCmd);
void OLED_SetPos(unsigned char ucIdxX, unsigned char ucIdxY);
...
void OLED_CLS(void){
  ...
  for (y = 0; y < 8; y++){
    ...
    OLED_WrCmd(0x01);
    OLED_WrCmd(0x10);
    for (x = 0; x < OLED_MAX_COLUMN_NUM; x++)
      OLED_WrDat(0);
  }
}
...
void SetStartColumn(unsigned char ucData){
  OLED_WrCmd(0x00 + ucData % 16);
    // Set Lower Column Start Address for Page Addressing Mode
  // Default => 0x00
  OLED_WrCmd(0x10 + ucData / 16);
...
void OLED_Init(void){
  ...
  DelayMs(100); //从上电到下面开始初始化要有足够的时间，即等待RC复位完毕
...
返 回 值  : NONE
 *****************************************************************************/
void OLED_P6x8Char(unsigned char ucIdxX, unsigned char ucIdxY, unsigned char
  ucData){
...
返 回 值  : NONE
 *****************************************************************************/
void OLED_P6x8Str(unsigned char ucIdxX, unsigned char ucIdxY, unsigned char
  ucDataStr[]){
 ...
 *****************************************************************************/
/*
void OLED_P8x16Str(unsigned char ucIdxX, unsigned char ucIdxY, unsigned char ucDataStr[])
{
unsigned char i, j, ucDataTmp;
 ...
 *****************************************************************************/
/*
void OLED_P14x16Str(unsigned char ucIdxX,unsigned char ucIdxY,unsigned char ucDataStr[])
{
unsigned char i, j;
unsigned short int usAddress;
 ...
 *****************************************************************************/
/*
void OLED_PXx16MixStr(unsigned char ucIdxX, unsigned char ucIdxY, unsigned char ucDataStr[])
{
 ...
 *****************************************************************************/
/*
void OLED_PrintChar(unsigned char ucIdxX, unsigned char ucIdxY, char cData)
{
unsigned short int i, j, k, usData;

if(cData < 0)
{
OLED_P6x8Char(ucIdxX, ucIdxY, '-');
usData = (unsigned short int)(-cData);
}
else
{
OLED_P6x8Char(ucIdxX, ucIdxY, '+');
...
返 回 值  : none
 *****************************************************************************/
void OLED_PrintShort(unsigned char ucIdxX, unsigned char ucIdxY, short int
  sData){
 ...
 *****************************************************************************/
/*
void OLED_PrintImage(unsigned char *pucTable, unsigned short int usRowNum, unsigned short int usColumnNum)
{
...
OLED_SetPos(0,(unsigned char)i); //设置剩下的行显示的起始坐标
usRowTmp = i << 3;       //计算当前所在行的下标
for(j = 0; j < usColumnNum; j++) //列扫描
{
ucData = 0;
