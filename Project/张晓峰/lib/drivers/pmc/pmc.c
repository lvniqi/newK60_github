...
void enter_vlpr(char lpwui_value)
{
   ...
   /* Reduce system clock to < 2MHz */
    ...
    if(lpwui_value){
       SMC_PMCTRL = (SMC_PMCTRL_LPWUI_MASK       // set LPWUI
                    | SMC_PMCTRL_RUNM(2));       // set RUNM = 0b10     
   
     } else {
       SMC_PMCTRL = (!SMC_PMCTRL_LPWUI_MASK            // clear LPWUI
                   | SMC_PMCTRL_RUNM(2));             // set RUNM = 0b10 
     }       
     ...
     while((PMC_REGSC & (SMC_PMSTAT & 0x04))==0);    // 0 MCU is not in VLPR mode
...
void exit_vlpr(void)
{
    ...
    while (PMC_REGSC & (SMC_PMSTAT & 0x04)); // 0 MCU is not in VLPR mode
                                              // 1 MCU is in VLPR mode
...
void enter_vlps(char lpwui_value)
{
 ...
 //   mcg_pee_2_blpi();    // you don't need to chage to lower frequency for VLPS
    ...
    if(lpwui_value){
       SMC_PMCTRL = (SMC_PMCTRL_LPWUI_MASK            // set LPWUI
                  | SMC_PMCTRL_STOPM(2));           // set LPLLSM = 0b10
     } else {
       SMC_PMCTRL = (!SMC_PMCTRL_LPWUI_MASK           // set LPWUI
                  | SMC_PMCTRL_STOPM(2));           // set LPLLSM = 0b10
     }       
...
void enter_vlls3(void)
{
   ...
   // SMC_PMPROT = SMC_PMPROT_AVLLS3_MASK;
        
    /* Set the LPLLSM field to 0b101 for VLLS3 mode  */
    SMC_PMCTRL =  SMC_PMCTRL_STOPM(5);           // set LPLLSM = 0b101
    disable_ports();    
...
void enter_vlls2(void)
{
   ...
   // SMC_PMPROT = SMC_PMPROT_AVLLS2_MASK;
        
    /* Set the LPLLSM field to 0b110 for VLLS2 mode */
    SMC_PMCTRL =  SMC_PMCTRL_STOPM(6);           // set LPLLSM = 0b110
    disable_ports();    
...
void enter_vlls1(void)
{
   ...
   // SMC_PMPROT = SMC_PMPROT_AVLLS1_MASK;
        
    /* Set the LPLLSM field to 0b111 for VLLS1 mode  */
    SMC_PMCTRL =  SMC_PMCTRL_STOPM(7);           // set LPLLSM = 0b111
    disable_ports();    
...
void enter_stop_mode(char mode)
{
    ...
    SMC_PMPROT = 0x37;               //This value enables all possible power modes.
 
    SMC_PMCTRL = SMC_PMCTRL_LPWUI_MASK | SMC_PMCTRL_STOPM(mode) ;  
    stop();         
}
  ...
  if (RCM_SRS1 & RCM_SRS1_SW_MASK)
	...
	if (RCM_SRS0 & RCM_SRS0_WAKEUP_MASK){
       ...
       // used by initialization software for proper power mode recovery.
          if ((SMC_PMCTRL & SMC_PMCTRL_STOPM_MASK) == 0)
             printf("[outSRS]Pin Reset wakeup from Normal Stop\n");
	        if ((SMC_PMCTRL & SMC_PMCTRL_STOPM_MASK) == 2)
